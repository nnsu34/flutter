...
class _FloorListState extends State<FloorList> {
  ...
  Widget build(BuildContext context) {
    if (isLoading) {
      return Center(
        child: CircularProgressIndicator(
          valueColor: AlwaysStoppedAnimation<Color>(Colors.grey.withOpacity(0.3)), // 옅은 회색으로 설정
        ),
      );
    }
    ...
    return _buildHorizontalFloors(floors, totalPeople);
  }

  // 층 목록을 생성하는 함수, 각 층의 비율을 계산하여 전달
  Widget _buildHorizontalFloors(List<Map<String, dynamic>> floors, int totalPeople) {
    return Container(
